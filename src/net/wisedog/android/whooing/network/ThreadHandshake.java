...
public class ThreadHandshake extends Thread {
	...
	public void run() {
		if(!isAfterAuth){	//Before Auth
			...
			if(result != null){
			    ...
			    msg.what = AppDefine.MSG_REQ_AUTH;
			    msg.obj = result;
			...
			boolean result = true;
			if(mSecondToken == null || AppDefine.PIN == null){
				result = false;
			}else{
				result = thirdHandshake(mSecondToken, AppDefine.PIN);
			}	
			if(result == false){
				Message msg = new Message();		
				msg.what = AppDefine.MSG_FAIL;
				mHandler.sendMessage(msg);
			}
		}
	...
	public String initHandshake(){
		String url = "https://whooing.com/app_auth/request_token?app_id="
						+ AppDefine.APP_ID+"&app_secret=" + AppDefine.APP_SECRET;
	...
	private boolean thirdHandshake(String secondToken, String pin){
		...
		String url = "https://whooing.com/app_auth/access_token?app_id="+ AppDefine.APP_ID+
				"&app_secret="+ AppDefine.APP_SECRET + "&token="+secondToken+"&pin="+pin;
		...
		SharedPreferences prefs = mActivity.getSharedPreferences(AppDefine.SHARED_PREFERENCE,
				Activity.MODE_PRIVATE);
		SharedPreferences.Editor editor = prefs.edit();
		editor.putString(AppDefine.KEY_SHARED_TOKEN_SECRET, token_secret);
		editor.putInt(AppDefine.KEY_SHARED_USER_ID, user_id);
		editor.putString(AppDefine.KEY_SHARED_TOKEN, realToken);
		editor.commit();
		
		AppDefine.TOKEN_SECRET = token_secret;
		AppDefine.USER_ID = user_id;
		AppDefine.REAL_TOKEN = realToken;
		
		Message msg = new Message();		
		msg.what = AppDefine.MSG_AUTH_DONE;
		mHandler.sendMessage(msg);
		return true;
	}
}